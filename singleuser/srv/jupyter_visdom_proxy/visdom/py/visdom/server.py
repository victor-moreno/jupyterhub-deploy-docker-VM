...
class Application(tornado.web.Application):
    def __init__(self, port=DEFAULT_PORT, base_url='',
                 ...
                 user_credential=None, use_frontend_client_polling=False, proxy_url=''):
        ...
        tornado_settings['static_url_prefix'] = self.proxy_url + self.base_url + "/static/"
...
def start_server(port=DEFAULT_PORT, hostname=DEFAULT_HOSTNAME,
                 ...
                 use_frontend_client_polling=False, proxy_url=DEFAULT_PROXY_URL):
    print("It's Alive!")
    app = Application(port=port, base_url=base_url, env_path=env_path,
                      readonly=readonly, user_credential=user_credential,
                      use_frontend_client_polling=use_frontend_client_polling, proxy_url=proxy_url)
...
def main(print_func=None):
    ...
    parser.add_argument('-proxy_url', metavar='proxy_url', type=str,
                        default=DEFAULT_PROXY_URL,
                        help='base url for proxy (default = /).')
    ...
    # Process proxy_url
    proxy_url = FLAGS.proxy_url if FLAGS.proxy_url != DEFAULT_PROXY_URL else ""

    if proxy_url[len(proxy_url)-1:] == '/':
        proxy_url = proxy_url[:-1]

    assert proxy_url == '' or proxy_url.startswith('/'), \
        'proxy_url should start with /'
    assert proxy_url == '' or not proxy_url.endswith('/'), \
        'proxy_url should not end with / as it is appended automatically'
    ...
    start_server(port=FLAGS.port, hostname=FLAGS.hostname, base_url=base_url,
                 ...
                 use_frontend_client_polling=FLAGS.use_frontend_client_polling, proxy_url=proxy_url)
